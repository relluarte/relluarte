...
import { Home, FileText, ShoppingCart, Users, Package, Menu, X, LogOut, Store as StoreIcon, Building2, Calculator } from 'lucide-react';
import { UserRole, Store } from '../types';

interface SidebarProps {
  setCurrentView: (view: 'dashboard' | 'proposals' | 'sales' | 'clients' | 'products' | 'stores' | 'calculator') => void;
  userRole: UserRole;
  onLogout: () => void;
  stores: Store[];
...
export const Sidebar: React.FC<SidebarProps> = ({ setCurrentView, userRole, onLogout, stores, selectedStore, setSelectedStore, isMobileMenuOpen = false, setIsMobileMenuOpen }) => {
  ...
  const handleNavClick = (view: 'dashboard' | 'proposals' | 'sales' | 'clients' | 'products' | 'stores' | 'calculator') => {
      setCurrentView(view);
      setMenuOpen(false);
  }
  ...
  return (
    <>
      ...
      >
        ...
        {/* Sidebar */}
        <div className={`relative w-80 max-w-[85vw] bg-background-secondary h-full shadow-2xl transform transition-transform duration-300 ease-out ${
